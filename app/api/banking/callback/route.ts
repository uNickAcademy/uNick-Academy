...
export async function GET(req: Request) {
  ...
  if (!isGoCardlessConfigured() || !entityId) {
    return NextResponse.redirect(`${origin}/ustawienia/bank?error=config`)
  }
  ...
  if (!pending?.requisition_id) {
    return NextResponse.redirect(`${origin}/ustawienia/bank?error=norequisition`)
  }
  ...
  try {
    ...
    if (!requisition.accounts || requisition.accounts.length === 0) {
      return NextResponse.redirect(`${origin}/ustawienia/bank?error=noaccounts`)
    }
    ...
    return NextResponse.redirect(`${origin}/ustawienia/bank?connected=1`)
  } catch {
    return NextResponse.redirect(`${origin}/ustawienia/bank?error=sync`)
  }
}
