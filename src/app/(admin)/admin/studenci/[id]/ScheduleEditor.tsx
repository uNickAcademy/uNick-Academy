...
import { Pencil, X, Repeat, Plus, Trash2, CalendarRange, CalendarOff, Link2, Banknote, Wallet, RefreshCw } from 'lucide-react'
...
// Ten sam mechanizm co edycja harmonogramu w panelu Studenci/Firmy
// (/api/admin/students/schedule) — nienaruszający zapis linku/stawki/
// prowadzącego na już zaplanowanych lekcjach, plus osobna, świadoma
// regeneracja przyszłej serii terminów. Dostępne bezpośrednio z karty
// ucznia (Klient 360°), żeby nie trzeba było wracać do listy.
export function ScheduleEditor({ student, teacherOptions }: { student: ScheduleStudent; teacherOptions: TeacherOpt[] }) {
  const [open, setOpen] = useState(false)
  return (
    <>
      <button onClick={() => setOpen(true)}
        className="flex items-center gap-1.5 text-xs font-semibold text-[#23479E] hover:underline">
        <Pencil size={13} />Edytuj harmonogram
      </button>
      {open && <ScheduleModal student={student} teacherOptions={teacherOptions} onClose={() => setOpen(false)} />}
    </>
  )
}

function ScheduleModal({ student, teacherOptions, onClose }: { student: ScheduleStudent; teacherOptions: TeacherOpt[]; onClose: () => void }) {
  ...
  const initialSlots = student.courseConfig?.slots?.length ? student.courseConfig.slots : (student.inferredSlot ? [student.inferredSlot] : [])
  const [slots, setSlots] = useState<Slot[]>(initialSlots.length > 0 ? initialSlots : [{ day: 1, time: '17:00', durationMin: 60 }])
  const [startDate, setStartDate] = useState(student.courseConfig?.startDate ?? today)
  const [endDate, setEndDate] = useState(student.courseConfig?.endDate ?? defaultCourseEndDate(today))
  const [ongoing, setOngoing] = useState(student.courseConfig != null && student.courseConfig.endDate == null)
  const effectiveEndDate = ongoing ? undefined : endDate
  const [excludedDates, setExcludedDates] = useState<string[]>(student.courseConfig?.excludedDates ?? [])
  const [newExcluded, setNewExcluded] = useState('')
  const [meetingUrl, setMeetingUrl] = useState(student.meetingUrl)
  const [lessonPrice, setLessonPrice] = useState(student.lessonPrice != null ? String(student.lessonPrice) : '')
  const [teacherRate, setTeacherRate] = useState(student.teacherRate != null ? String(student.teacherRate) : '')
  ...
  function payload(applySchedule: boolean) {
    return { studentId: student.id, teacherId, meetingUrl, slots, startDate, endDate: ongoing ? null : endDate, excludedDates, lessonPrice, teacherRate, applySchedule }
  }
  ...
  async function applySchedule() {
    ...
    if (!confirm(
      `Zastąpić przyszłe terminy ${student.name}?\n\n` +
