...
export function checkReferralEligibility({ referrer, candidate }) {
  ...
  // B2B/corporate accounts (e.g. Democo) sit outside the referral programme
  // entirely - they can neither refer others nor redeem a code themselves.
  if (referrer.companyId || candidate.companyId) {
...
export async function getStudentReferralOverview(userId) {
  ...
  // B2B/corporate accounts (e.g. Democo) are not part of the referral
  // programme - they don't refer others and can't redeem codes themselves.
  const programmeAvailable = !user.companyId
  ...
  return {
    ...
    canApplyReferralCode: programmeAvailable && !referralReceived && purchaseCount === 0,
