/**
 * GUS REGON (BIR1.1) lookup — read-only, fetches a company's official
 * registry data (name, REGON, address) by NIP.
 *
 * Setup required (CFO, one-time):
 *   1. Free API key at https://api.stat.gov.pl/Home/RegonApi (register with email)
 *   2. Add to Vercel env vars: GUS_API_KEY
 *
 * The service is a legacy SOAP API (WCF, SOAP 1.2): login → search → logout.
 * Each search returns an XML-escaped XML string nested inside the SOAP body,
 * which we parse in two passes.
 */
...
/** Look up a company's registry data by NIP (10 digits). */
